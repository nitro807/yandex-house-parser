...
- Для каждой найденной организации открывается её карточка; телефон, email и сайт сохраняются, если они опубликованы в Яндекс Картах.
- Карточки обрабатываются тремя параллельными воркерами. Число можно изменить переменной `CONTACT_WORKERS`.
