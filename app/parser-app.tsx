...
export function ParserApp() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const response = await fetch("/api/parse", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ url: url.trim(), max_organizations: 100 }) });
...
function Results({ result, onDownload }: { result: ParseResult; onDownload: () => void }) {
  return <div className="space-y-6">
    ...
    {result.warnings.length > 0 && <Alert className="rounded-2xl border-[#e6b85f] bg-[#fff8e7] text-[#49340f]"><TriangleAlert className="size-4 text-[#85530a]" /><AlertTitle className="text-[#49340f]">Обрати внимание</AlertTitle><AlertDescription className="text-[#654b1c]">{result.warnings.join(" ")}</AlertDescription></Alert>}
    <div className="min-w-0 overflow-hidden rounded-2xl border border-[#17211f]/10 bg-white [&_[data-slot=table-container]]:overflow-x-hidden"><div className="max-h-[calc(100vh-320px)] max-w-full overflow-y-auto"><Table className="w-full table-fixed text-[#17211f]"><TableHeader className="sticky top-0 z-10 bg-[#f0eee7] text-[#34413e]"><TableRow><TableHead className="w-12">№</TableHead><TableHead className="w-[38%]">Организация</TableHead><TableHead className="w-[24%]">Категория</TableHead><TableHead className="w-[26%]">Контакты</TableHead><TableHead className="w-16 text-center">Карта</TableHead></TableRow></TableHeader><TableBody>
      {result.organizations.map((item, index) => { const website = websiteUrl(item.website); return <TableRow key={item.id || `${item.name}-${index}`}><TableCell className="align-top text-[#687572]">{String(index + 1).padStart(2, "0")}</TableCell><TableCell className="min-w-0 whitespace-normal align-top [overflow-wrap:anywhere]"><p className="font-medium leading-5">{item.name}</p>{item.address && <p className="mt-1 text-xs leading-5 text-[#586763]">{item.address}</p>}{item.rating != null && <Badge className="mt-2 bg-[#e9f5dd] text-[#365b21]">★ {item.rating}</Badge>}</TableCell><TableCell className="min-w-0 whitespace-normal align-top text-sm leading-5 text-[#465450] [overflow-wrap:anywhere]">{item.category || "—"}</TableCell><TableCell className="min-w-0 whitespace-normal align-top text-sm leading-5 [overflow-wrap:anywhere]">{item.phones.map((phone) => <div key={phone}>{phone}</div>)}{website && <a href={website} target="_blank" rel="noreferrer" className="mt-1 inline-flex max-w-full items-center gap-1 text-[#a93420] hover:underline">Сайт <ArrowUpRight className="size-3 shrink-0" /></a>}{!item.phones.length && !website && "—"}</TableCell><TableCell className="align-top text-center">{item.yandex_url ? <Button asChild variant="ghost" size="icon" className="rounded-full text-[#263431]"><a href={item.yandex_url} target="_blank" rel="noreferrer" aria-label={`Открыть ${item.name} на Яндекс Картах`}><ArrowUpRight className="size-4" /></a></Button> : "—"}</TableCell></TableRow>; })}
